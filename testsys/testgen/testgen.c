...
void testDead() {
    ...
    for (int cache_size = 4, iter = 0; cache_size < 17000; cache_size *= 2, ++iter) {
        FILE *dead = NULL;
        sprintf(file_name, "testpool/%d-%d.tst", DEAD, iter);
        dead = fopen(file_name, "w");
        assert(dead);

        fprintf(dead, "%d 1000000 ", cache_size);
...
void testNoise() {
    ...
    for (int cache_size = 4, iter = 0; cache_size < 17000; cache_size *= 2, ++iter) {
        FILE *noise = NULL;
        sprintf(file_name, "testpool/%d-%d.tst", NOISE, iter);
        noise = fopen(file_name, "w");
        assert(noise);

        srand(seed);
        fprintf(noise, "%d 1000000 ", cache_size);
        for (int i = 0; i < 1000000; ++i) {
...
void testWhisper() {
    ...
    for (int cache_size = 4, iter = 0; cache_size < 17000; cache_size *= 2, ++iter) {
        FILE *whisper = NULL;
        sprintf(file_name, "testpool/%d-%d.tst", WHISPER, iter);
        whisper = fopen(file_name, "w");
        assert(whisper);

        srand(seed);
        fprintf(whisper, "%d 1000000 ", cache_size);
        for (int i = 0; i < 1000000; ++i) {
...
void testSetsFar() {
    ...
    for (int cache_size = 4, iter = 0; cache_size < 17000; cache_size *= 2, ++iter) {
        int reqs = 0;
        FILE *sets_far = NULL;
        sprintf(file_name, "testpool/%d-%d.tst", SETS_F, iter);
        sets_far = fopen(file_name, "w");
        assert(sets_far);

        srand(seed);
        fprintf(sets_far, "%d 1000000 ", cache_size);
        while (reqs < 1000000) {
...
void testSetsClose() {
    ...
    for (int cache_size = 4, iter = 0; cache_size < 17000; cache_size *= 2, ++iter) {
        ...
        sprintf(file_name, "testpool/%d-%d.tst", SETS_C, iter);
        sets_close = fopen(file_name, "w");
        assert(sets_close);

        srand(seed);
        fprintf(sets_close, "%d 1000000 ", cache_size);
        while (reqs < 1000000) {
...
void testOutburstsClose() {
    ...
    for (int cache_size = 4, iter = 0; cache_size < 17000; cache_size *= 2, ++iter) {
        ...
        srand(seed);
        fprintf(outbursts_close, "%d 1000000 ", cache_size);
        while (reqs < 1000000) {
            int timer = rand() % 7 + 1;
...
void testOutburstsFar() {
    ...
    for (int cache_size = 4, iter = 0; cache_size < 17000; cache_size *= 2, ++iter) {
        ...
        srand(seed);
        fprintf(outbursts_far, "%d 1000000 ", cache_size);
        while (reqs < 1000000) {
            int timer = rand() % 51 + 9;
...
void testSetsOutbursts() {
    ...
    for (int cache_size = 4, iter = 0; cache_size < 17000; cache_size *= 2, ++iter) {
        int reqs = 0;
        FILE *set_burst = NULL;
        sprintf(file_name, "testpool/%d-%d.tst", SETS_OUTBURSTS, iter);
        set_burst = fopen(file_name, "w");
        assert(set_burst);

        srand(seed);
        fprintf(set_burst, "%d 1000000 ", cache_size);
        while (reqs < 1000000) {
...
void testBasePulse() {
    ...
    for (int cache_size = 4, iter = 0; cache_size < 17000; cache_size *= 2, ++iter) {
        ...
        sprintf(file_name, "testpool/%d-%d.tst", BASE_PULSE, iter);
        grow_burst = fopen(file_name, "w");
        assert(grow_burst);

        srand(seed);
        fprintf(grow_burst, "%d 1000000 ", cache_size);
        while (reqs < 1000000) {
            int timer = rand() % 7 + 2;
...
void testDiapPulse() {
    ...
    for (int cache_size = 4, iter = 0; cache_size < 17000; cache_size *= 2, ++iter) {
        ...
        bool up = true;
        FILE *dyn_diap = NULL;
        sprintf(file_name, "testpool/%d-%d.tst", DIAP_PULSE, iter);
        dyn_diap = fopen(file_name, "w");
        assert(dyn_diap);

        srand(seed);
        fprintf(dyn_diap, "%d 1000000 ", cache_size);
        while (reqs < 1000000) {
